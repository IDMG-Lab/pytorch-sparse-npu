...
[[maybe_unused]] uint32_t NNOPBASE_Ind2ptr = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnInd2ptrTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "Ind2ptr";
    char inputDesc[] = {1};
    char outputDesc[] = {1};
    char attrDesc[] = {1};
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, indices, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&m), sizeof(int64_t), 0, kNnopbaseInt));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "indices", true));
