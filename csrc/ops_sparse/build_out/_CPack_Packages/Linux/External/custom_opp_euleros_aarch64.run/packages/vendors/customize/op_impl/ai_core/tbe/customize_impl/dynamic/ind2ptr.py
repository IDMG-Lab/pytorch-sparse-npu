...
DTYPE_MAP = {"float32": ["DT_FLOAT", "float"],
    ...
    "float4_e2m1":["DT_FLOAT4_E2M1", "float4_e2m1_t"],
    "float4_e1m2":["DT_FLOAT4_E1M2", "float4_e1m2_t"]}
...
def _build_args(indices_in__, ptr_out_, M):
    __inputs__ = []
...
@tbe_register.register_operator("Ind2ptr", trans_bool_to_s8=False)
...
def ind2ptr(indices_in__, ptr_out_, M, kernel_name="ind2ptr", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, ptr_out_, M)
    ...
    origin_func_name = "ind2ptr"
    ascendc_src_dir_ex = "ind2ptr"
    ascendc_src_dir = "ind2ptr"
    ascendc_src_file = "ind2ptr.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Ind2ptr, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Ind2ptr"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[indices_in__], origin_outputs = [ptr_out_],\
...
def op_select_format(indices_in__, ptr_out_, M, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, ptr_out_, M)
    result = check_op_cap("op_select_format", "Ind2ptr", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(indices_in__, ptr_out_, M, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, ptr_out_, M)
    result = check_op_cap("get_op_specific_info", "Ind2ptr", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
