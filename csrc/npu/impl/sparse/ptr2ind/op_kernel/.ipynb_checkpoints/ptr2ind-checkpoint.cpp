...
public:
    ...
    __aicore__ inline void Init(GM_ADDR ptr_gm, GM_ADDR ind_gm, GM_ADDR tiling_gm) {
        ...
        indOut.SetGlobalBuffer((__gm__ int64_t *)ind_gm + blockStart, blockCount);
...
private:
    __aicore__ inline void Compute(uint32_t tileIdx) {
        auto out = outQueue.AllocTensor<int64_t>();
    ...
    __aicore__ inline void CopyOut(uint32_t tileIdx) {
        auto out = outQueue.DeQue<int64_t>();
        DataCopy(indOut[tileIdx * TILE_LENGTH], out, TILE_LENGTH);
